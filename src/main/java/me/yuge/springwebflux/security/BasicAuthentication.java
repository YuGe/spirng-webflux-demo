...
public class BasicAuthentication implements Function<String, Mono<Authentication>> {
    ...
    public Mono<Authentication> apply(String authorization) {
        ...
        return userRepository.findByUsername(tokens[0])
                .filter(user -> passwordEncoder.matches(tokens[1], user.getPassword()))
                .switchIfEmpty(Mono.error(new BadCredentialsException("Login or password not correct")))
